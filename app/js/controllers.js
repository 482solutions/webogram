...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppWelcomeController',
    function($scope, $location, MtpApiManager, ChangelogNotifyService, LayoutSwitchService) {
  ...
  .controller('AppLoginController',
    function($scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, LayoutSwitchService, WebPushApiManager, TelegramMeWebService, _) {
      ...
      function initPhoneCountry() {
        ...
        MtpApiManager.invokeApi('help.getNearestDc', {}, { dcID: 2, createNetworker: true }).then(
          function(nearestDcResult) {
      ...
      $scope.checkPassword = function() {
        $scope.progress.enabled = true
        return PasswordManager.check($scope.password, $scope.credentials.password, options).then(
          saveAuth,
          function(error) {
            $scope.progress.enabled = false
            switch (error.type) {
    ...
    function($q, qSync, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, InAPIManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, ErrorService, AppRuntimeManager, HttpsMigrateService, LayoutSwitchService, LocationParamsService, AppStickersManager) {
      ...
      function updateCurDialog() {
        ...
        if ($routeParams.p && $routeParams.p.charAt(0) == '@') {
          ...
          peerStringPromise = AppPeersManager.resolveUsername($routeParams.p.substr(1)).then(
            function(peerID) {
  ...
  .controller('AppImDialogsController',
    function($scope, $location, $q, $timeout, $routeParams, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppProfileManager, AppPeersManager, PhonebookContactsService, ErrorService, AppRuntimeManager) {
      $scope.dialogs = []
      $scope.myResults = []
      $scope.foundPeers = []
      $scope.foundMessages = []
      ...
      function getDialogs(force) {
        ...
        if (searchMessages) {
          ...
          return searchTimeoutPromise.then(function() {
            ...
            return AppMessagesManager.getSearch(searchPeerID,
              $scope.search.query,
      ...
      function showMoreConversations() {
        ...
        if ($scope.search.query && $scope.search.query.length >= 2) {
          $timeout(function() {
            if (curJump != contactsJump) return
            MtpApiManager.invokeApi('contacts.search', { q: $scope.search.query, limit: 10 }).then(
              function(result) {
  ...
  .controller('AppImHistoryController',
    function($scope, $location, $timeout, $modal, $rootScope, toaster, _, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService, GeoLocationManager) {
      ...
      function updateHistoryPeer(preload) {
        ...
        if (preload) {
          ...
          safeReplaceObject($scope.state,
            { loaded: true, empty: !peerHistory.messages.length, mayBeHasMore: true })
      ...
      function showLessHistory() {
        ...
        var limit = 0
        var backLimit = 20
        AppMessagesManager.getHistory($scope.curDialog.peerID, minID, limit, backLimit).then(
      ...
      function showMoreHistory() {
        ...
        var getMessagesPromise = inputMediaFilter
          ? AppMessagesManager.getSearch($scope.curDialog.peerID,
            '',
            inputMediaFilter,
            maxID,
            limit)
        ...
        getMessagesPromise.then(function(historyResult) {
          ...
          if (historyResult.history.length) {
            ...
            if (AppMessagesManager.regroupWrappedHistory(peerHistory.messages,
              historyResult.history.length + 1)) {
              $scope.$broadcast('messages_regroup')
            }
          }
      ...
      function loadHistory(forceRecent) {
        ...
        var getMessagesPromise = inputMediaFilter
          ...
          : AppMessagesManager.getHistory($scope.curDialog.peerID,
            maxID,
            limit,
            backLimit,
            prerenderedLen)
  ...
  .controller('AppImSendController',
    function($rootScope, $q, $scope, $timeout, MtpApiManager, Storage, AppProfileManager, AppChatsManager, AppUsersManager, AppPeersManager, AppDocsManager, AppStickersManager, AppMessagesManager, AppInlineBotsManager, MtpApiFileManager, DraftsManager, RichTextProcessor) {
      ...
      function updateMentions() {
        ...
        if (peerID < 0 && !AppPeersManager.isBroadcast(peerID)) {
          if (AppPeersManager.isChannel(peerID)) {
            ...
            chatParticipantsPromise = AppProfileManager.getChatFull(-peerID)
              .then(function(chatFull) {
        ...
        $q.all({ pop: inlineBotsPromise, chat: chatParticipantsPromise }).then(function(result) {
          ...
          angular.forEach(ids, function(userID) {
            ...
            SearchIndexManager.indexObject(userID,
              AppUsersManager.getUserSearchText(userID),
              mentionIndex)
          })
        ...
        AppProfileManager.getPeerBots(peerID).then(function(peerBots) {
          ...
          angular.forEach(peerBots, function(peerBot) {
            ...
            angular.forEach(peerBot.commands, function(description, command) {
              ...
              SearchIndexManager.indexObject(value,
                botSearchText + ' ' + command + ' ' + description,
                commandsIndex)
            })
          })
      ...
      function getDraft() {
        if ($scope.curDialog.peerID) {
          ...
          if (editMessageID) {
            draftDataPromise = AppMessagesManager.getMessageEditData(editMessageID)
      ...
      function checkInlinePattern(message) {
        ...
        inlineBotPromise.then(function(inlineBot) {
          ...
          getInlineResultsTO = $timeout(function() {
            ...
            AppInlineBotsManager.getInlineResults($scope.curDialog.peerID,
              inlineBot.id,
              query,
              inlineBot.geo,
  ...
  .controller('AppLangSelectController',
    function($scope, _, Storage, ErrorService, AppRuntimeManager) {
  ...
  .controller('PhotoModalController',
    function($q, $scope, $rootScope, $modalInstance, AppPhotosManager, AppMessagesManager, AppPeersManager, AppWebPagesManager, PeersSelectService, ErrorService) {
      ...
      AppMessagesManager.getSearch(peerID, inputQuery, inputFilter, 0, 1000)
        .then(function(searchCachedResult) {
      ...
      function loadMore() {
        ...
        return loadingPromise = AppMessagesManager.getSearch(peerID, inputQuery, inputFilter, maxID)
  ...
  .controller('UserpicModalController',
    function($q, $scope, $rootScope, $modalInstance, MtpApiManager, AppPhotosManager, AppUsersManager, AppPeersManager, AppMessagesManager, ApiUpdatesManager, PeersSelectService, ErrorService) {
      $scope.photo = AppPhotosManager.wrapForFull($scope.photoID)
      $scope.photo.thumb = {
      ...
      function loadMore() {
        ...
        return loadingPromise = AppPhotosManager.getUserPhotos($scope.userID, maxID)
          .then(function(userpicResult) {
      ...
      $scope.forward = function() {
        PeersSelectService.selectPeer({ confirm_type: 'FORWARD_PEER', canSend: true })
  ...
  .controller('ChatpicModalController',
    function($q, $scope, $rootScope, $modalInstance, MtpApiManager, AppPhotosManager, AppChatsManager, AppPeersManager, AppMessagesManager, ApiUpdatesManager, PeersSelectService, ErrorService) {
      $scope.photo = AppPhotosManager.wrapForFull($scope.photoID)
      $scope.photo.thumb = {
        location: AppPhotosManager.choosePhotoSize($scope.photo, 0, 0).location
      }
      ...
      $scope['delete'] = function() {
        ErrorService.confirm({ type: 'PHOTO_DELETE' }).then(function() {
          ...
          apiPromise.then(function(updates) {
            ...
            $modalInstance.dismiss()
            $rootScope.$broadcast('history_focus',
              { peerString: AppChatsManager.getChatString($scope.chatID) })
          })['finally'](function() {
  ...
  .controller('VideoModalController',
    function($scope, $rootScope, $modalInstance, PeersSelectService, AppMessagesManager, AppDocsManager, AppPeersManager, ErrorService) {
  ...
  .controller('GameModalController',
    function($q, $scope, $rootScope, $modalInstance, AppPhotosManager, AppMessagesManager, AppPeersManager, AppGamesManager, PeersSelectService, ErrorService) {
      ...
      $scope.forward = function(withMyScore) {
        PeersSelectService.selectPeer({ canSend: true, confirm_type: 'INVITE_TO_GAME' }).then(
          function(peerString) {
            var peerID = AppPeersManager.getPeerID(peerString)
  ...
  .controller('UserModalController',
    function($scope, $location, $rootScope, $modalInstance, AppProfileManager, $modal, AppUsersManager, MtpApiManager, NotificationsManager, AppPhotosManager, AppMessagesManager, AppPeersManager, PeersSelectService, ErrorService) {
      ...
      $scope.flushHistory = function(justClear) {
        ErrorService.confirm({ type: justClear ? 'HISTORY_FLUSH' : 'HISTORY_FLUSH_AND_DELETE' })
          .then(function() {
      ...
      $scope.shareContact = function() {
        PeersSelectService.selectPeer({ confirm_type: 'SHARE_CONTACT_PEER', canSend: true }).then(
          function(peerString) {
            var peerID = AppPeersManager.getPeerID(peerString)
            AppMessagesManager.sendOther(peerID, {
              _: 'inputMediaContact',
  ...
  .controller('ChatModalController',
    function($scope, $modalInstance, $location, $timeout, $rootScope, $modal, AppUsersManager, AppChatsManager, AppProfileManager, AppPhotosManager, MtpApiManager, MtpApiFileManager, NotificationsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, ContactsSelectService, ErrorService) {
  ...
  .controller('ChannelModalController',
    function($scope, $timeout, $rootScope, $modal, AppUsersManager, AppChatsManager, AppProfileManager, AppPhotosManager, MtpApiManager, MtpApiFileManager, NotificationsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, ContactsSelectService, ErrorService) {
      $scope.chatFull = AppChatsManager.wrapForFull($scope.chatID, {})
      $scope.settings = { notifications: true }
      ...
      $scope.leaveChannel = function() {
        return ErrorService.confirm({ type: $scope.isMegagroup ? 'MEGAGROUP_LEAVE' : 'CHANNEL_LEAVE' })
          .then(function() {
      ...
      $scope.deleteChannel = function() {
        return ErrorService.confirm({ type: $scope.isMegagroup ? 'MEGAGROUP_DELETE' : 'CHANNEL_DELETE' })
          .then(function() {
      ...
      $scope.inviteToChannel = function() {
        var disabled = []
        angular.forEach(($scope.chatFull.participants || {}).participants || [],
  ...
  .controller('SettingsModalController',
    function($rootScope, $scope, $timeout, $modal, AppUsersManager, AppChatsManager, AppPhotosManager, MtpApiManager, Storage, NotificationsManager, MtpApiFileManager, PasswordManager, ApiUpdatesManager, ChangelogNotifyService, LayoutSwitchService, WebPushApiManager, AppRuntimeManager, ErrorService, _) {
      ...
      var contentSettingsPromise = MtpApiManager.invokeApi('account.getContentSettings', {}).then(
      ...
      Storage.get('notify_nodesktop',
        'send_ctrlenter',
        'notify_volume',
        'notify_novibrate',
        'notify_nopreview',
        'notify_nopush').then(function(settings) {
  ...
  .controller('ChangelogModalController', function($scope, $modal) {
    ...
    if (!$scope.lastVersion) {
      ...
      $scope.lastVersion = versionParts[0] + '.' + versionParts[1] + '.' + Math.max(0,
        versionParts[2] - 1)
    }
  ...
  .controller('ProfileEditModalController',
    function($scope, $modalInstance, AppUsersManager, MtpApiManager) {
      $scope.profile = {}
      $scope.error = {}
  ...
  .controller('SessionsListModalController',
    function($scope, $q, $timeout, _, MtpApiManager, ErrorService, $modalInstance) {
  ...
  .controller('IncognitoKeysModalController',
    function($rootScope, $scope, $timeout, $modal, AppUsersManager, AppChatsManager, AppIncognitoStateManager, InAPIManager, Storage, NotificationsManager, MtpApiFileManager, PasswordManager, ApiUpdatesManager, ChangelogNotifyService, LayoutSwitchService, WebPushApiManager, AppRuntimeManager, ErrorService, _) {
      $scope.accounts = AppIncognitoStateManager.getAccountsList();
      ...
      $scope.createKeyChain = function() {
        $modal.open({
          templateUrl: templateUrl('create_key_chair_modal'),
          controller: 'KeyChairModalController',
          windowClass: 'md_simple_modal_window mobile_modal'
        })
      }
  ...
  .controller('KeyChairModalController', function($rootScope, $scope, AppIncognitoStateManager) {
    ...
    $scope.createKeyChain = function() {
      $scope.isLoading = true;
      AppIncognitoStateManager.createAccount($scope.keyChair.name).then(function() {
        $rootScope.$broadcast('incognitoAPIUpdate', 'accountsList');
        $scope.isLoading = false;
        $scope.$close();
      })
    }
    ...
    $scope.importKeyChain = function() {
      $scope.isLoading = true;
      AppIncognitoStateManager.importAccount($scope.keyChair.name || '',
        $scope.keyChair.privateKey || ' ').then(
        function() {
          $rootScope.$broadcast('incognitoAPIUpdate', 'accountsList');
          $scope.isLoading = false;
          $scope.$close();
  ...
  .controller('AccountModalController',
    function($scope, $q, $timeout, _, AppIncognitoStateManager) {
  ...
  .controller('AccountsListModalController',
    function($rootScope, $scope, $q, $timeout, _, AppIncognitoStateManager) {
  ...
  .controller('PasswordUpdateModalController',
    function($scope, $q, _, PasswordManager, MtpApiManager, ErrorService, $modalInstance) {
      ...
      $scope.$watch('passwordSettings.new_password', function(newValue) {
        ...
        if (!len) {
          ...
          $scope.passwordSettings.hint = newValue.charAt(0) + (new Array(len - 1)).join('*') + newValue.charAt(
            len - 1)
        }
        $scope.$broadcast('value_updated')
      })
  ...
  .controller('ContactsModalController',
    function($scope, $rootScope, $timeout, $modal, $modalInstance, MtpApiManager, AppPeersManager, AppUsersManager, ErrorService) {
      $scope.contacts = []
      $scope.foundPeers = []
      $scope.search = {}
      $scope.slice = { limit: 20, limitDelta: 20 }
      ...
      function updateContacts(query) {
        ...
        if (query && query.length >= 2) {
          $timeout(function() {
            if (curJump != jump) return
            MtpApiManager.invokeApi('contacts.search', { q: query, limit: 10 })
              .then(function(result) {
                AppUsersManager.saveApiUsers(result.users)
                if (curJump != jump) return
  ...
  .controller('PeerSelectController',
    function($scope, $modalInstance, $q, AppPeersManager, ErrorService) {
      $scope.selectedPeers = {}
  ...
  .controller('ChatCreateModalController',
    function($scope, $modalInstance, $rootScope, MtpApiManager, AppUsersManager, AppChatsManager, ApiUpdatesManager) {
      $scope.group = { name: '' }
  ...
  .controller('ChannelEditModalController',
    function($q, $scope, $modalInstance, $rootScope, MtpApiManager, AppUsersManager, AppChatsManager, AppProfileManager, ApiUpdatesManager) {
  ...
  .controller('ChatInviteLinkModalController',
    function(_, $scope, $timeout, $modalInstance, AppChatsManager, AppProfileManager, ErrorService) {
  ...
  .controller('ImportContactModalController',
    function($scope, $modalInstance, $rootScope, AppUsersManager, ErrorService, PhonebookContactsService) {
  ...
  .controller('PhonebookModalController',
    function($scope, $modalInstance, $rootScope, AppUsersManager, PhonebookContactsService, ErrorService) {
      $scope.search = {}
      $scope.phonebook = []
      $scope.selectedContacts = {}
      $scope.selectedCount = 0
      ...
      PhonebookContactsService.getPhonebookContacts().then(function(phonebook) {
        for (var i = 0; i < phonebook.length; i++) {
          SearchIndexManager.indexObject(i,
            phonebook[i].first_name + ' ' + phonebook[i].last_name + ' ' + phonebook[i].phones.join(
            ' '),
            searchIndex)
        }
  ...
  .controller('StickersetModalController',
    function($scope, $rootScope, $modalInstance, MtpApiManager, RichTextProcessor, AppStickersManager, AppDocsManager, AppMessagesManager, LocationParamsService) {
      $scope.slice = { limit: 20, limitDelta: 20 }
      ...
      $scope.share = function() {
        LocationParamsService.shareUrl('https://t.me/addstickers/' + $scope.stickerset.short_name,
          $scope.stickerset.title)
      }
    })

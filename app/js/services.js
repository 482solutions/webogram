...
angular.module('myApp.services', ['myApp.i18n', 'izhukov.utils'])
  ...
  .service('AppUsersManager',
    function($rootScope, $modal, $modalStack, $filter, $q, qSync, MtpApiManager, RichTextProcessor, ServerTimeManager, Storage, _) {
      var users = {}
      var usernames = {}
      var userAccess = {}
      ...
      function saveApiUser(apiUser, noReplace) {
        ...
        if (apiUser.first_name) {
          ...
          apiUser.rFullName = apiUser.last_name ? RichTextProcessor.wrapRichText(apiUser.first_name + ' ' + (apiUser.last_name || ''),
            { noLinks: true, noLinebreaks: true }) : apiUser.rFirstName
        } else {
          apiUser.rFirstName = RichTextProcessor.wrapRichText(apiUser.last_name,
            { noLinks: true, noLinebreaks: true }) || apiUser.rPhone || _('user_first_name_deleted')
          apiUser.rFullName = RichTextProcessor.wrapRichText(apiUser.last_name,
            { noLinks: true, noLinebreaks: true }) || apiUser.rPhone || _('user_name_deleted')
        }
        ...
        if (cachedPhotoLocations[userID] !== undefined) {
          safeReplaceObject(cachedPhotoLocations[userID],
            apiUser && apiUser.photo && apiUser.photo.photo_small || { empty: true })
        }
      }
        ...
        switch (update._) {
          ...
          case 'updateUserPhoto':
            ...
            if (user) {
              ...
              if (cachedPhotoLocations[userID] !== undefined) {
                safeReplaceObject(cachedPhotoLocations[userID],
                  update.photo && update.photo.photo_small || { empty: true })
              }
  ...
  .service('AppChatsManager',
    function($q, $rootScope, $modal, _, MtpApiManager, AppUsersManager, AppPhotosManager, RichTextProcessor) {
      var chats = {}
      var usernames = {}
      var channelAccess = {}
      var megagroups = {}
      ...
      function saveApiChat(apiChat) {
        ...
        if (cachedPhotoLocations[apiChat.id] !== undefined) {
          safeReplaceObject(cachedPhotoLocations[apiChat.id],
            apiChat && apiChat.photo && apiChat.photo.photo_small || { empty: true })
        }
      }
      ...
      function wrapForFull(id, fullChat) {
        ...
        if (chatFull.participants &&
          ...
          chatFull.participants.participants = wrapParticipants(id,
  ...
  .service('AppPeersManager', function($q, qSync, AppUsersManager, AppChatsManager, MtpApiManager) {
    ...
    function resolveUsername(username) {
      ...
      return MtpApiManager.invokeApi('contacts.resolveUsername', { username: username }).then(
  ...
  .service('AppProfileManager',
    function($q, $rootScope, AppUsersManager, AppChatsManager, AppMessagesIDsManager, AppPeersManager, AppPhotosManager, NotificationsManager, MtpApiManager, ApiUpdatesManager, RichTextProcessor, Storage) {
      var botInfos = {}
      var chatsFull = {}
      ...
      function getChannelFull(id, force) {
        ...
        return chatFullPromises[id] = MtpApiManager.invokeApi('channels.getFullChannel', {
          ...
          if (fullChannel.pinned_msg_id) {
            fullChannel.pinned_msg_id = AppMessagesIDsManager.getFullMessageID(fullChannel.pinned_msg_id,
              id)
          }
        ...
        switch (update._) {
          ...
          case 'updateChannelPinnedMessage':
            ...
            if (fullChannel !== undefined) {
              fullChannel.pinned_msg_id = AppMessagesIDsManager.getFullMessageID(update.id,
                channelID)
  ...
  .service('AppPhotosManager',
    function($modal, $window, $rootScope, MtpApiManager, MtpApiFileManager, AppUsersManager, FileManager) {
      var photos = {}
  ...
  .service('AppWebPagesManager',
    function($modal, $sce, $window, $rootScope, MtpApiManager, AppPhotosManager, AppDocsManager, RichTextProcessor) {
      var webpages = {}
      var pendingWebPages = {}
      ...
      function wrapForFull(webPageID) {
        ...
        var embedHtml = '<' + embedTag + ' src="' + encodeEntities(webPage.embed_url) + '" type="' + encodeEntities(
          embedType) + '" frameborder="0" border="0" webkitallowfullscreen mozallowfullscreen allowfullscreen width="' + full.width + '" height="' + full.height + '" style="width: ' + full.width + 'px; height: ' + full.height + 'px;"></' + embedTag + '>'
  ...
  .service('AppGamesManager',
    function($modal, $sce, $window, $rootScope, MtpApiManager, AppPhotosManager, AppDocsManager, RichTextProcessor) {
      ...
      function wrapForFull(gameID, msgID, embedUrl) {
        ...
        var embedHtml = '<' + embedTag + ' src="' + encodeEntities(embedUrl) + '" type="' + encodeEntities(
          embedType) + '" frameborder="0" border="0" webkitallowfullscreen mozallowfullscreen allowfullscreen width="' + full.width + '" height="' + full.height + '" style="width: ' + full.width + 'px; height: ' + full.height + 'px;" sandbox="allow-scripts allow-same-origin"></' + embedTag + '>'
  ...
  .service('AppDocsManager',
    function($sce, $rootScope, $modal, $window, $q, $timeout, RichTextProcessor, MtpApiFileManager, FileManager, qSync) {
      var docs = {}
      var docsForHistory = {}
      var windowW = $(window).width()
      var windowH = $(window).height()
      ...
      function wrapForHistory(docID) {
        ...
        doc.withPreview = !Config.Mobile && doc.mime_type.match(
          /^image\/(gif|png|jpeg|jpg|bmp|tiff)/) ? 1 : 0
      ...
      function downloadDoc(docID, toFileEntry) {
        ...
        var downloadPromise = MtpApiFileManager.downloadFile(doc.dc_id,
          inputFileLocation,
          doc.size,
          {
        ...
        downloadPromise.then(function(blob) {
          ...
          historyDoc.progress.percent = Math.max(1,
            Math.floor(100 * progress.done / progress.total))
      ...
      function saveDocFile(docID) {
        ...
        FileManager.chooseSave(getFileName(doc), ext, doc.mime_type)
          .then(function(writableFileEntry) {
            if (writableFileEntry) {
  ...
  .service('AppStickersManager',
    function($q, $rootScope, $modal, _, FileManager, MtpApiManager, AppDocsManager, Storage, ApiUpdatesManager) {
      var started = false
      var applied = false
  ...
  .service('AppInlineBotsManager',
    function(qSync, $q, $rootScope, toaster, Storage, ErrorService, MtpApiManager, AppMessagesManager, AppMessagesIDsManager, AppDocsManager, AppPhotosManager, AppGamesManager, RichTextProcessor, AppUsersManager, AppPeersManager, LocationParamsService, PeersSelectService, GeoLocationManager) {
      var inlineResults = {}
        ...
        return MtpApiManager.invokeApi('messages.getInlineBotResults', {
          ...
          if (botResults.switch_pm) {
            botResults.switch_pm.rText = RichTextProcessor.wrapRichText(botResults.switch_pm.text,
              { noLinebreaks: true, noLinks: true })
          }
    ...
    function($rootScope, MtpNetworkerFactory, AppUsersManager, AppChatsManager, AppPeersManager, MtpApiManager) {
      ...
      function popPendingPtsUpdate(channelID) {
        ...
        console.log(dT(),
          ...
          goodPts,
          curState.pendingPtsUpdates.slice(0, goodIndex + 1))
      ...
      function getDifference() {
        ...
        MtpApiManager.invokeApi('updates.getDifference',
          { pts: updatesState.pts, date: updatesState.date, qts: -1 },
          {
            timeout: 0x7fffffff
      ...
      function getChannelDifference(channelID) {
        ...
        MtpApiManager.invokeApi('updates.getChannelDifference', {
          ...
          console.log(dT(),
            'applying',
            differenceResult.other_updates.length,
          ...
          console.log(dT(),
            'applying',
            differenceResult.new_messages.length,
      ...
      function processUpdate(update, options) {
        ...
        if (update._ == 'updateNewMessage' ||
          ...
          if (message.from_id && !AppUsersManager.hasUser(message.from_id,
            message.pFlags.post/* || channelID*/) && (reason = 'author') ||
            fwdHeader.from_id && !AppUsersManager.hasUser(fwdHeader.from_id,
              !!fwdHeader.channel_id) && (reason = 'fwdAuthor') ||
            fwdHeader.channel_id && !AppChatsManager.hasChat(fwdHeader.channel_id,
              true) && (reason = 'fwdChannel') ||
        ...
        if (update.pts) {
          ...
          if (newPts < update.pts) {
            console.warn(dT(),
              ...
              curState,
              update,
              channelID && AppChatsManager.getChat(channelID))
          ...
          if (seqStart != curState.seq + 1) {
            if (seqStart > curState.seq) {
              console.warn(dT(),
                'Seq hole',
                curState,
                curState.syncPending && curState.syncPending.seqAwaiting)
      ...
      function attach() {
        ...
        MtpApiManager.invokeApi('updates.getState', {}, { noErrorBox: true })
  ...
  .service('StatusManager',
    function($timeout, $rootScope, MtpApiManager, AppUsersManager, IdleManager) {
      var toPromise
  ...
  .service('NotificationsManager',
    function($rootScope, $window, $interval, $q, $modal, _, toaster, MtpApiManager, AppPeersManager, AppChatsManager, AppUsersManager, IdleManager, Storage, AppRuntimeManager, FileManager, WebPushApiManager) {
      ...
      function updateNotifySettings() {
        Storage.get('notify_nodesktop',
          ...
          'notify_novibrate',
          'notify_nopreview',
          'notify_nopush').then(function(updSettings) {
      ...
      function notify(data) {
        ...
        if ('Notification' in window) {
          ...
          notification = navigator.mozNotification.createNotification(data.title,
            data.message || '',
            data.image || '')
      ...
      function playSound(volume) {
        ...
        var obj = $('#notify_sound').html(
          '<audio autoplay="autoplay" mozaudiochannel="notification">' +
  ...
  .service('ChangelogNotifyService',
    function(Storage, $rootScope, $modal, $timeout, MtpApiManager, ApiUpdatesManager) {
  ...
  .service('LocationParamsService',
    function(qSync, $rootScope, $routeParams, AppPeersManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppMessagesIDsManager, MtpApiManager, ApiUpdatesManager, PeersSelectService, AppStickersManager, ErrorService) {
        ...
        if (matches = url.match(/^resolve\?domain=(.+?)(?:&(start|startgroup|post|game)=(.+))?$/)) {
          AppPeersManager.resolveUsername(matches[1]).then(function(peerID) {
            ...
            if (matches[2] == 'start') {
              ...
              params.messageID = AppMessagesIDsManager.getFullMessageID(parseInt(matches[3]),
                -peerID)
            }
      ...
      function handleActivityMessage(name, data) {
        ...
        if (name == 'share' && data.url) {
          ...
          if (phones.length) {
            AppUsersManager.importContact(phones[0], firstName, lastName)
          ...
          PeersSelectService.selectPeers({ confirm_type: 'EXT_SHARE_PEER', canSend: true }).then(
            function(peerStrings) {
  ...
  .service('DraftsManager',
    function($rootScope, qSync, MtpApiManager, ApiUpdatesManager, AppMessagesIDsManager, AppChatsManager, AppPeersManager, RichTextProcessor, Storage, ServerTimeManager) {
